...
const COLUMN_VISIBILITY_STORAGE_KEY = "employee-index-visible-columns";
...
const tableFilters = ref({
    ...
    company_id: { value: null, matchMode: "equals" },
...
const availableColumns = computed(() => {
    ...
    return [
        { label: trans("Company"), value: "company_name" },
        { label: trans("Employee name"), value: "name" },
        { label: trans("Email"), value: "email" },
        { label: trans("Status"), value: "active" },
...
const quickStats = computed(() => {
    ...
    return [
        {
            label: trans("Employees on this page"),
        ...
        },
        {
            ...
            value: companyCount,
            caption: trans("Companies represented in the current view"),
        },
    ];
});
...
const activeFilters = computed(() => {
    ...
    if (tableFilters.value.company_id.value) {
        const company = props.companyOptions.find(
            (option) => option.value === tableFilters.value.company_id.value
        );
    ...
    if (tableFilters.value.name.value) {
        filters.push({
            key: "name",
            label: `${trans("Employee name")}: ${tableFilters.value.name.value}`,
        });
    }
    ...
    if (tableFilters.value.active.value !== null) {
        filters.push({
            key: "active",
            label: `${trans("Status")}: ${
                tableFilters.value.active.value ? trans("Active") : trans("Inactive")
            }`,
        });
    }
...
const fetchEmployees = async () => {
    ...
    try {
        const response = await employeeService.list({
            ...
            company_id: tableFilters.value.company_id.value || undefined,
            name: tableFilters.value.name.value || undefined,
            email: tableFilters.value.email.value || undefined,
            active: tableFilters.value.active.value ?? undefined,
...
const buildRowActions = (employee) => [
    {
        label: trans("Edit"),
        icon: "pi pi-pencil",
        command: () => router.get(route("employees.edit", employee.id)),
    },
    {
        label: trans("Delete"),
        icon: "pi pi-trash",
        severity: "danger",
        command: () => removeEmployee(employee),
    },
    {
        label: employee.active ? trans("Deactivate") : trans("Activate"),
        icon: employee.active ? "pi pi-eye-slash" : "pi pi-check-circle",
        severity: employee.active ? "danger" : "success",
        command: () => toggleEmployeeActiveStatus(employee),
    },
];
            ...
            <Card class="app-card border-0">
                <template #content>
                    <DataTable
                        ...
                        :global-filter-fields="['company_name', 'name', 'email']"
                    ...
                    >
                        ...
                        <Column
                            ...
                            :header="$t('Company')"
                            sortable
                            v-if="isColumnVisible('company_name')"
                        >
