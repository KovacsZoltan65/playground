...
const DEFAULT_VISIBLE_COLUMN_KEYS = ["name", "guard_name", "permissions_count", "updated_at"];
...
const availableColumns = computed(() => {
    ...
    return [
        { label: trans("Role name"), value: "name" },
        { label: trans("Guard"), value: "guard_name" },
        { label: trans("Assigned permissions"), value: "permissions_count" },
...
const quickStats = computed(() => {
    ...
    return [
        {
            label: trans("Roles on this page"),
            value: roles.value.length,
            caption: trans("Rows currently loaded in the table"),
        },
        {
            label: trans("Configured on this page"),
            value: roles.value.filter((role) => role.permissions_count > 0).length,
            caption: trans("Roles that already have at least one permission"),
        },
        {
            label: trans("Guard"),
...
const activeFilters = computed(() => {
    ...
    if (tableFilters.value.name.value) {
        filters.push({
            key: "name",
            label: `${trans("Role name")}: ${tableFilters.value.name.value}`,
        });
    }
...
const removeRole = async (role) => {
    const accepted = await requestConfirmation(confirm, {
        message: trans("Delete :name?", { name: role.name }),
        header: trans("Delete"),
    ...
    try {
        ...
        await fetchRoles();
        showSuccessToast(trans("Role deleted successfully."));
    } catch (error) {
...
const removeSelectedRoles = async () => {
    ...
    const accepted = await requestConfirmation(confirm, {
        message: trans("Delete :count selected roles?", {
    ...
    try {
        await roleService.bulkDestroy(selectedRoles.value.map((role) => role.id));
        selectedRoles.value = [];
        ...
        await fetchRoles();
        showSuccessToast(trans("Roles deleted successfully."));
    } catch (error) {
        showErrorToast(error?.response?.data?.message);
    }
};
...
const refreshRoles = async () => {
    try {
        await fetchRoles();
        showSuccessToast(trans("Roles refreshed."));
    } catch (error) {
        showErrorToast(error?.response?.data?.message);
    }
};
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>{{ $t("Roles") }}</template>
                ...
                <template #content>
                    <div class="mb-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                        <div>
                            ...
                            <p class="mt-2 max-w-3xl text-slate-500">
                                {{ $t("Maintain role definitions and the permission bundles assigned to each one.") }}
                            </p>
                        </div>
                    ...
                    >
                        ...
                        <template #empty>
                            <div class="py-10 text-center">
                                <div class="text-lg font-medium text-slate-700">
                                    {{ $t("No roles found for the current filters.") }}
                                </div>
                                <div class="mt-2 text-sm text-slate-500">
                        ...
                        <Column
                            ...
                            sortable
                            v-if="isColumnVisible('permissions_count')"
                        >
                            <template #body="{ data }">
                                <span class="text-slate-600">{{ data.permissions_count }}</span>
                            </template>
                        </Column>
