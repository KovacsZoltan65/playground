...
const loadPermission = async () => {
    ...
    try {
        const response = await permissionService.show(props.permissionId);
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>{{ $t("Edit Permission") }}</template>
                    ...
                    <div class="mb-8 flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-2 text-slate-500">
                                {{ $t("Edit the selected permission record and save your changes.") }}
                            </p>
                        </div>
